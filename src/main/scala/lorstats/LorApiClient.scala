...
class LorApiClient(client: Client[IO], riotToken: String) extends CirceEntityDecoder {
  ...
  def getCards: IO[NonEmptyList[Card]] = {
    val sets = NonEmptyList.of("set1", "set2", "set3", "set4", "set5", "set6")
  ...
  def getMastersRank(gameName: String): IO[Option[Int]] = {
    riotApiRequest[MastersPlayers](s"lor/ranked/v1/leaderboards").map(_.players.find(_.name == gameName).map(_.rank))
  }
